...
/**
 * Adds a new logcat console appender to this logger config.
 *
 ...
 */
fun LoggerConfigBuilder.logcatAppender( // @formatter:off
    pattern: String,
    formatter: LogFormatter = LogFormatter.plain,
    filter: LogFilter = LogFilter.always
) { // @formatter:on
    appenders += LogcatAppender(pattern, formatter, filter)
}
...
actual fun LoggerConfigBuilder.platformConsoleAppender(
    ...
    filter: LogFilter
) = logcatAppender(pattern, formatter, filter)